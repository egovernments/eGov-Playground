...
public class ComplaintElasticSearchQueryTest {
	...
	public void before(){
		...
		complaint4.setComplaintDuration(8);
	...
	public void listComplaintBasedOnCrn(){
		final SearchQuery searchQuery = new NativeSearchQueryBuilder().
				withQuery(matchQuery("crn", "00010-2016-HG")).build();
		System.out.println();
		System.out.println("-----------------------------------------------");
		...
		final Iterable<Complaint> complaintsList = complaintService.search(searchQuery);
		complaints.forEach(complaint -> System.out.println(complaint.getId() + " " + complaint.getCrn()));
		complaintsList.forEach(complaint -> System.out.println(complaint.getId() + " " + complaint.getCrn()));
	}
	...
	public void searchByCrn(){
		...
		Page<Complaint> complaints = complaintService.findByCrn("00030-2016-WV", new PageRequest(0, 10));
		complaints.forEach(complaint -> System.out.println(complaint.getId() + "-" + complaint.getCrn()));
	}

	@Test
	public void searchComplaintById(){
		System.out.println();
		System.out.println("-----------------------------------------------");
		System.out.println("REPOSITORY FIND ONE");
		Complaint complaint = complaintService.findOne("00002");
		System.out.println(complaint.getId()+"-"+complaint.getComplaintType());
	}

	//Test case to match full text
	@Test
	public void returnRecordsWhereCompleteFieldMatch() {
		final SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(matchQuery("details", "Complaint is Registered").operator(AND)).build();
		final List<Complaint> complaints = elasticsearchTemplate.queryForList(searchQuery, Complaint.class);
		System.out.println();
		System.out.println("---------------------------------------------");
	...
	@Test
	public void returnRecordsWherePartOfFieldMatch() {
		final SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(matchQuery("details", "process")).build();
		final List<Complaint> complaints = elasticsearchTemplate.queryForList(searchQuery, Complaint.class);
		System.out.println();
		System.out.println("---------------------------------------------");
		System.out.println("PART OF THE DETAILS SEARCH");
		complaints.forEach(complaint -> System.out.println(complaint.getId()+"-"+complaint.getDetails()));
	}

	@Test
	public void returnRecordsBasedOnMultipleFieldValues() {
		final SearchQuery searchQuery = new NativeSearchQueryBuilder()
		.withQuery(QueryBuilders.multiMatchQuery("in")
				.field("details").field("complaintType")).build();

		final List<Complaint> complaints = elasticsearchTemplate.queryForList(searchQuery, Complaint.class);
		System.out.println();
		System.out.println("-----------------------------------------------");
		System.out.println("QUERY ON MULTIPLE FIELDS");
		complaints.forEach(complaint -> System.out.println(complaint.getDetails()+" - "+ complaint.getComplaintType()));
	}


	@Test
	public void returnRecordsBasedOnNestedFieldValue() {
		final QueryBuilder builder = nestedQuery("escalations", boolQuery().must(termQuery("escalations.escalatedFrom", "Clerk")));
	...
	public void fuzzinessquery() {		
		...
		.withQuery(matchQuery("details", "Complain is in Prosess")
				.operator(AND)
	...
	public void phraseSearch() {
		final SearchQuery searchQuery = new NativeSearchQueryBuilder().
				withQuery(QueryBuilders.matchPhraseQuery("details", "Complaint is Process").slop(1)).build();
		final List<Complaint> complaints = elasticsearchTemplate.queryForList(searchQuery, Complaint.class);
		System.out.println();
		System.out.println("-----------------------------------------------");
	...
	@Test
	public void returnAllRecordsWithSorting(){
		System.out.println();
		System.out.println("-----------------------------------------------");
		...
		SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(QueryBuilders.matchAllQuery())
				.withSort(new FieldSortBuilder("complaintDuration").order(SortOrder.DESC)).build();
		final List<Complaint> complaints = elasticsearchTemplate.queryForList(searchQuery, Complaint.class);
		complaints.forEach(complaint -> System.out.println(complaint.getId() + "-" + complaint.getComplaintDuration()));
	}
